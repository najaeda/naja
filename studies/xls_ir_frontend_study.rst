...
.. code-block:: text
   ...
   src/nl/formats/common/frontend/
     ...
     SNLDesignBuilder.h/.cpp   width-exact SNL/DB0 emission primitives
...
Status on 2026-09-17: dependency/interchange spike completed against XLS
commit ``0a7c502ccaaa650cb2da1929c536c4ab100eafda``. The retained study record
contains representative function, block, hierarchy, tuple/array, proc, and
malformed fixtures together with the dependency decision and support matrix.
The disposable Bazel probe and standalone CMake harness were deliberately not
retained. The preliminary decision is to use a pinned bridge executable for
the first vertical slice; the experimental JSON boundary is not yet the
...
Status on 2026-09-17: the first extraction slice introduces
``SNLDesignBuilder`` and routes SystemVerilog add, subtract, and mux emission
through it. The common target has no slang dependency and is covered in both
CMake and Bazel.
