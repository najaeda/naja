...
def check_generated_netlist(generated_path: Path) -> None:
    ...
    regfile_text = generated_module_text(generated_text, "ibex_register_file_ff")
    if regfile_text is None:
        raise SystemExit(
            "generated netlist does not contain module ibex_register_file_ff. "
            "Regenerate ibex_naja.v with the rebuilt Naja Python module before rerunning "
            "helloworld_sim."
        )

    waddr_one_bits = decoded_waddr_one_bits(regfile_text)
    if not {0, 1}.issubset(waddr_one_bits):
        raise SystemExit(
...
def generated_module_text(generated_text: str, module_name: str) -> str | None:
    module_match = re.search(rf"^\s*module\s+{re.escape(module_name)}\b", generated_text, re.MULTILINE)
    if module_match is None:
        return None
    end_match = re.search(r"^\s*endmodule\b", generated_text[module_match.end():], re.MULTILINE)
    if end_match is None:
        return generated_text[module_match.start():]
    return generated_text[module_match.start():module_match.end() + end_match.end()]


_BASED_LITERAL_RE = re.compile(r"\b\d+\s*'\s*[sS]?\s*[bBoOdDhH]\s*[0-9a-fA-F_xzXZ?]+\b")


def decoded_waddr_one_bits(regfile_text: str) -> set[int]:
    one_bits: set[int] = set()
    for statement in regfile_text.split(";"):
        for bit in range(5):
            if statement_compares_waddr_bit_to_one(statement, bit):
                one_bits.add(bit)
    return one_bits


def statement_compares_waddr_bit_to_one(statement: str, bit: int) -> bool:
    if re.search(rf"\bwaddr_a_i\s*\[\s*{bit}\s*\]", statement):
        return any(verilog_literal_has_one_at_bit(literal.group(0), 0)
                   for literal in _BASED_LITERAL_RE.finditer(statement))
    if re.search(r"\bwaddr_a_i\b(?!\s*\[)", statement):
        return any(verilog_literal_has_one_at_bit(literal.group(0), bit)
                   for literal in _BASED_LITERAL_RE.finditer(statement))
    return False


def verilog_literal_has_one_at_bit(literal: str, bit: int) -> bool:
    match = re.fullmatch(
        r"\s*(?P<width>\d+)\s*'\s*[sS]?\s*(?P<base>[bBoOdDhH])\s*"
        r"(?P<digits>[0-9a-fA-F_xzXZ?]+)\s*",
        literal,
    )
    if match is None:
        return False
    ...
    return bool(int(digits, radix) & (1 << bit))
