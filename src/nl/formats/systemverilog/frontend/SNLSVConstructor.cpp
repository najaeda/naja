          ...
          if (strippedAssignedLHS &&
            ...
            for (const auto* leafExpr : concatLeafExpressions) {
              ...
              if (sameLhs(leafExpr, &lhsExpr)) {
                if (trackedOffset.has_value()) {
                  std::ostringstream reason;
                  reason << "duplicate sequential concatenation assignment leaf for '"
                ...
                if (leafBits.size() != lhsBits.size()) {
                  std::ostringstream reason;
                  reason << "sequential concatenation assignment width mismatch for tracked LHS '"
                         << describeLHSForDiagnostics(lhsExpr) << "'";
                  failureReason = reason.str();
                  return false;
                }
            ...
            if (trackedOffset.has_value()) {
              ...
              if (assignedBits.size() != concatWidth) {
                std::ostringstream reason;
                reason << "failed to lower sequential concatenation assignment for '"
            ...
            if (resolveAssignmentLHSBits(
              ...
              if (isConcreteSubset &&
                ...
                if (assignedBits.size() != assignedLHSBits.size()) {
                  std::ostringstream reason;
                  reason << "failed to lower concrete sequential partial assignment for LHS '"
      ...
      for (const auto* itemExpr : item.expressions) {
        ...
        if (insideCase) {
          if (!buildCaseInsideMatchBit(
              ...
              !exprMatchBit) {
            std::ostringstream reason;
            reason << "unable to resolve always_comb case inside item match for "
    ...
    bool tryApplyCombinationalRangeSelectAssignmentForLhs(
      ...
      switch (rangeExpr.getSelectionKind()) {
        ...
        case slang::ast::RangeSelectionKind::IndexedDown: {
          ...
          if (!selectorWidth || !*selectorWidth) {
            std::ostringstream reason;
            reason << "unable to resolve dynamic indexed range-select base width in always_comb assignment: "
                   << describeExpression(assignedLHS);
            failureReason = reason.str();
            return false;
          }
          ...
          if (!resolveExpressionBits(
              ...
              selectorBits.size() != static_cast<size_t>(*selectorWidth)) {
            std::ostringstream reason;
            reason << "unable to resolve dynamic indexed range-select base bits in always_comb assignment: "
                   << describeExpression(assignedLHS);
            failureReason = reason.str();
            return false;
          }
          ...
          while (index != end + step) {
            ...
            if (!equalsIndexBit) {
              failureReason =
                "failed to build dynamic indexed range-select decode while lowering always_comb assignment";
              return false;
            }
          ...
          if (!sawValidCandidate) {
            std::ostringstream reason;
            reason << "dynamic indexed range-select out of range in always_comb assignment: "
                   << describeExpression(assignedLHS);
            failureReason = reason.str();
            return false;
          }
    ...
    bool lowerSequentialMultiAssignmentConditional(
      ...
      for (const auto* lhsExpr : rawResetLHSExpressions) {
        ...
        if (!appendFlattenedConcatenationLHSExpressions(
              *lhsExpr,
              resetLHSExpressions)) {
          failureReason = "failed to flatten reset-branch LHS expressions";
          return false; // LCOV_EXCL_LINE
        }
      }
        ...
        if (collectAssignedLHSExpressions(
          ...
          for (const auto* lhsExpr : rawElseLHSExpressions) {
            ...
            if (!appendFlattenedConcatenationLHSExpressions(
                  *lhsExpr,
                  elseLHSExpressions)) {
              failureReason = "failed to flatten else-branch LHS expressions";
              return false; // LCOV_EXCL_LINE
            }
          }
      ...
      for (const auto* lhsExpr : conditionalLHSExpressions) {
        ...
        if (!useAsyncResetDFFRN && !useAsyncResetDFFRE && !useAsyncResetDFFSE) {
          ...
          if (!createMux2Instance(
                ...
                resetSourceRange)) {
            std::ostringstream reason;
            reason << "unable to build reset mux for '"
                   << describeLHSForDiagnostics(*lhsExpr) << "'";
            failureReason = reason.str();
            return false;
          }
