...
struct NLID final {
  ...
  NLID(DBID dbID, LibraryID libraryID):
    type_(NLID::Type::Library),
    dbID_(dbID),
    libraryID_(libraryID)
  {}
  ...
  constexpr NLID(Type type, DBID dbID, LibraryID libraryID, DesignID designID, DesignObjectID id, DesignObjectID instanceID, Bit bit):
    type_(type),
    dbID_(dbID),
    libraryID_(libraryID),
    designID_(designID),
  ...
  friend bool operator<(const NLID& lid, const NLID& rid) {
