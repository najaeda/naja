        ...
        if file.endswith(".py"):
            ...
            try:
                ...
                if process_return.returncode != 0:
                    logging.error(f"Test {test_path} failed with return code {process_return.returncode}!")
                    failure = True

            except subprocess.TimeoutExpired:
                logging.error(f"Test {test_path} timed out after {timeout_seconds} seconds!")
                failure = True
            except Exception as e:
                logging.error(f"An error occurred while running {test_path}: {e}")
                failure = True
