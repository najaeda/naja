...
VHDL type analysis. Scalar registers and internal-signal pipeline scheduling are now implemented
(see below). Extend the proof
to type-checked scalar expressions, ascending and descending vectors, a small hierarchy, and retained-variable state beyond the temporary-variable proof below.
