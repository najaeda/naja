...
class SNLDesignCloneTest2(unittest.TestCase):
  def setUp(self):
    ...
    constant1 = naja.SNLInstance.create(self.design, logic1, "constant1")
    constant1.getInstTerm(logic1Out).setNet(n0.getBit(1))
  ...
  def testClone(self):
    ...
    n0b1 = n0.getBit(1)
    self.assertIsNotNone(n0b1)
    self.assertIsInstance(n0b1, naja.SNLBusNetBit)
    self.assertTrue(n0b1.isConstant1())
    self.assertEqual(1, n0b1.getBit())
    self.assertEqual(n0, n0b1.getBus())
    self.assertEqual(1, sum(1 for c in n0b1.getComponents()))
