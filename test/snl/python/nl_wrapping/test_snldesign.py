...
class SNLDesignTest(unittest.TestCase):
  def setUp(self):
    ...
    self.lib = snl.NLLibrary.create(db)
  ...
  def test0(self):
    ...
    self.assertIsNone(snl.NLUniverse.get().getTopDB())
    self.assertIsNone(snl.NLUniverse.get().getTopDesign())
    snl.NLUniverse.get().setTopDesign(design)
    self.assertTrue(design.isTopDesign())
    self.assertEqual(design, snl.NLUniverse.get().getTopDesign())
    self.assertEqual(design.getDB(), snl.NLUniverse.get().getTopDB())
    with self.assertRaises(RuntimeError) as context: snl.NLUniverse.get().setTopDesign(self.lib)
