...
TEST_F(DNLTests, SNLDataAccessWith2levelsOfHierarchy) {
  ...
  SNLDesign* submod = SNLDesign::create(library, NLName("submod"));
  auto subinTerm =
      SNLScalarTerm::create(submod, SNLTerm::Direction::Input, NLName("subin"));
...
TEST_F(DNLTests, SNLDataAccessWith3levelsOfHierarchyAndIsoDBWithMultiDriverMT) {
  ...
  auto inTerm =
      SNLScalarTerm::create(mod, SNLTerm::Direction::Input, NLName("in"));
  auto inTerm2 =
      SNLScalarTerm::create(mod, SNLTerm::Direction::Input, NLName("in2"));
  auto outTerm =
      SNLScalarTerm::create(mod, SNLTerm::Direction::Output, NLName("out"));
  // Create a sub module snl with one input and one output
  SNLDesign* submod = SNLDesign::create(library, NLName("submod"));
  ...
  auto subsuboutTerm = SNLScalarTerm::create(
      subsubmod, SNLTerm::Direction::Output, NLName("subsubout"));
  auto subsuboutTerm2 = SNLScalarTerm::create(
      subsubmod, SNLTerm::Direction::Output, NLName("subsubout2"));
  SNLInstance* subsubinst =
      SNLInstance::create(submod, subsubmod, NLName("subsubinst"));
  // Connect the output of the second sub module to the output of the first sub
  // module
...
// Based on last test, only with multi driver
TEST_F(DNLTests,
       SNLDataAccessWith3levelsOfHierarchyAndIsoDBWithMultiDriverNonMT) {
  // Create one module snl with one input and one output
  setenv("NON_MT", "", 1);
  NLUniverse* univ = NLUniverse::create();
  NLDB* db = NLDB::create(univ);
  ...
  auto subinTerm =
      SNLScalarTerm::create(submod, SNLTerm::Direction::Input, NLName("subin"));
  auto subinTerm2 = SNLScalarTerm::create(submod, SNLTerm::Direction::Input,
                                          NLName("subin2"));
  auto suboutTerm = SNLScalarTerm::create(submod, SNLTerm::Direction::Output,
                                          NLName("subout"));
  SNLInstance* subinst = SNLInstance::create(mod, submod, NLName("subinst"));
  ...
  SNLInstance* subsubinst =
      SNLInstance::create(submod, subsubmod, NLName("subsubinst"));
  SNLInstance* subsubinst2 =
      SNLInstance::create(submod, subsubmod, NLName("subsubinst2"));
  // Connect the output of the second sub module to the output of the first sub
  // module
  auto subOutNet = SNLScalarNet::create(submod, NLName("submodnet"));
  ...
  subinst->getInstTerm(suboutTerm)->setNet(outNet);
  // Connect the input of the second sub module to the input of the first
  // submodule module auto subsInNet = SNLScalarNet::create(submod);
  ...
  printf("Full path: %s\n", dnl->getTop()
                                ...
                                .getFullPath()
                                .c_str());
  SNLEquipotential equipotential = dnl->getTop()
                                       .getChildInstance(subinst)
                                       .getTerminalFromBitTerm(subinTerm)
                                       .getEquipotential();
  // Create the same equipotential with SNL
  std::vector<std::string> stringsPath;
  naja::DNL::DNLInstanceFull currentInstance = dnl->getTop().getChildInstance(
    subinst);
  while (currentInstance.isTop() == false) {
    stringsPath.push_back(currentInstance.getSNLInstance()->getName().getString());
  ...
  naja::NL::SNLNetComponentOccurrence occurrence(
      snlPath, dnl->getTop()
                   ...
                   .getTerminalFromBitTerm(subinTerm)
                   .getSnlBitTerm());
  naja::NL::SNLEquipotential equipotential2(occurrence);

  EXPECT_TRUE(equipotential == 
                equipotential2);
  ...
  EXPECT_EQ(dnl->getTop()
                .getChildInstance(subinst)
                .getTerminal(subinst->getInstTerm(subinTerm))
                .isNull(),
            false);
  EXPECT_EQ(dnl->getDNLNullInstance().isNull(), true);
  EXPECT_EQ(dnl->getDNLNullTerminal().isNull(), true);
