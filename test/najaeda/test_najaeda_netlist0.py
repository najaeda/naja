...
class NajaNetlistTest0(unittest.TestCase):
    ...
    def test_const_nets(self):
        ...
        net.set_constant(netlist.LogicValue.ZERO)
        self.assertTrue(net.is_const())
        self.assertTrue(net.is_const0())
        net1 = top.create_net('net1')
        net1.set_constant(netlist.LogicValue.ONE)
        self.assertTrue(net1.is_const())
        self.assertTrue(net1.is_const1())
        mixed = top.create_bus_net('mixed', 3, 0)
        mixed.set_constant(netlist.LogicVector("4'b10xz"))
        self.assertTrue(mixed.is_const())
        self.assertFalse(mixed.is_const0())
        self.assertFalse(mixed.is_const1())
        raw_top = universe.getTopDesign()
        regular_model = naja.SNLDesign.create(raw_top.getLibrary(), 'RegularModel')
        naja.SNLInstance.create(raw_top, regular_model, 'regular')
        regular = top.get_child_instance('regular')
        constants = list(top.get_constant_driver_child_instances())
        helpers = list(top.get_helper_child_instances())
        self.assertEqual([regular], list(top.get_regular_child_instances()))
        self.assertEqual([], list(top.get_assign_child_instances()))
        self.assertEqual(3, len(constants))
        self.assertEqual(constants, helpers)
        self.assertEqual(4, len(list(top.get_child_instances())))
        self.assertTrue(regular.is_regular())
        self.assertFalse(regular.is_constant_driver())
        self.assertTrue(all(instance.is_constant_driver() for instance in constants))
        self.assertTrue(all(not instance.is_regular() for instance in constants))
        net = net1
        top_o = top.create_output_term('O')
        self.assertIsNotNone(top_o)
        top_o.connect_lower_net(net)
        self.assertIsNotNone(top_o.get_lower_net())
        self.assertTrue(top_o.get_lower_net().is_const())
        self.assertTrue(top_o.get_lower_net().is_const1())
        equi = top_o.get_equipotential()
        self.assertIsNotNone(equi)
        self.assertListEqual(list(equi.get_top_terms()), [top_o])
