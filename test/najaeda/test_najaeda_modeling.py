...
class NajaNetlistModeling(unittest.TestCase):
    ...
    def test(self):
        ...
        o.connect(o_net)
        ck.connect(ck_net)
        and2_ins.get_term('I0').connect(i0_net)
        and2_ins.get_term('I1').connect(i1_net)
        and2_ins.get_term('Z').connect(and2_z_net)
        ff_ins.get_term('D').connect(and2_z_net)
        ff_ins.get_term('CK').connect(ck_net)
        ff_ins.get_term('Q').connect(o_net)
